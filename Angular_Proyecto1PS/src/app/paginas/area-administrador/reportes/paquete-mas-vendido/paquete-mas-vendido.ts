...
@Component({
  selector: 'app-paquete-mas-vendido',
  imports: [Reportes, FormsModule],
  templateUrl: './paquete-mas-vendido.html',
  styleUrl: './paquete-mas-vendido.css',
})
export class PaqueteMasVendido {
  ...
  buscarPorFechas(): void {
    ...
    this.reporteService.getPaqueteMasVendido(this.fechaInicial, this.fehcaFinal).subscribe({
  ...
  private buscarPDF(): void {
    this.reportePDFService.getPaqueteMasVendidoPDF(this.fechaInicial, this.fehcaFinal).subscribe({
      next: (pdf: Blob) => {
  ...
  descargarPdf(): void {
    ...
    a.href = url;
    a.download = 'reporte de paquete mas vendido.pdf';
    a.click();
